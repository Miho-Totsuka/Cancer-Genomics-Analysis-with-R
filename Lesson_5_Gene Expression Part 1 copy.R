...
CHOL_expression <- readRDS("CHOL_expression.rds")
...
CHOL_Data.frame <- as.data.frame(colData(CHOL_expression))
...
CHOL_Data.frame_patient <- CHOL_Data.frame |> 
  select("patient",
    "barcode",
    "shortLetterCode",
    "vital_status",
...
mRNA_CHOL <- assay(CHOL_expression)
...
mRNA_CHOL_dataframe <- as.data.frame(t(mRNA_CHOL)) #the t is important


#what changed?

mRNA_CHOL_dataframe$ID <- row.names(mRNA_CHOL_dataframe)

 #ads ID as variable

CHOL_Data.frame_patient$ID <- row.names(CHOL_Data.frame_patient)
 #same thing. 

combined_CHOL <- left_join(mRNA_CHOL_dataframe, CHOL_Data.frame_patient, by = c("ID"="ID"), copy =TRUE)
...
combined_CHOL <- combined_CHOL |> 
  filter(sample_type %in% c("Solid Tissue Normal", "Primary Tumor")) |> 
  mutate(sample_type = recode(sample_type,
    "Primary Tumor"= "Tumor",
    "Solid Tissue Normal"= "Normal"))

grep("ENSG00000171345", colnames(combined_CHOL), value = TRUE)


TCGA_CHOL_KRT19_TPM <- ggplot(combined_CHOL, aes(x=sample_type, y=ENSG00000171345.13, fill=sample_type))+
  geom_boxplot()+
  ...
  theme_classic()+
  labs(title = "Normal vs Tumor KRT19 Expression", x= "Sample", y = "KRT19 Counts 
...
ggsave(
  filename = "CHOL_KRT19_TPM.png",
  plot = TCGA_CHOL_KRT19_TPM,
  width = 7,
  height = 7,
  dpi = 300
)
